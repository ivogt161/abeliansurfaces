...
logging.basicConfig(format='%(asctime)s %(levelname)s: %(message)s', datefmt='%H:%M:%S',
    filename='drew_5.log', level=logging.DEBUG)
...
def nonmaximal_wrapper_for_drew_tests(row):
    ...
    try:
        ...
        probably_nonmaximal_primes = is_surjective(C, L=[5])
        return probably_nonmaximal_primes
    except ValueError:
        logger.critical("Failed; entries are {} and {}".format(row['data'][0].list(),row['data'][1].list()))
        raise
...
def run_drew_5():
    ...
    # df = df.iloc[int(1000):int(1005)].copy()

    df["data"] = df["data"].str.replace('^','**')
    df["data"] = df["data"].apply(eval_string)

    df['probably_nonmaximal_primes'] = df.apply(nonmaximal_wrapper_for_drew_tests, axis=int(1))
