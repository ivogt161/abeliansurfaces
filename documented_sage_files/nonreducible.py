r"""
TODO This file seems to only contain functions that can also be found in
``nonmaximal.py``. Perhaps some rearrangements of functions is in order.
...
def p_part(p, N):
    r"""
    TODO: This function is also in ``nonmaximal.py``, so we should keep
    a separate place for auxiliary files.
    """
    if N != 0:
